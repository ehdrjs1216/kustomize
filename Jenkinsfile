pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '621622519389.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
    awsecrRegistryCredential = 'credential-AWS-ECR'
    githubCredential = 'credential-github'
    gitEmail = 'ehdrjs1216@gmail.com'
    gitName = 'ehdrjs1216'
  }
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/ehdrjs1216/kustomize.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/ehdrjs1216/kustomize.git',
            branch: 'main'  
    ...
    stage('Push to Git Repository') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
             sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/ehdrjs1216/kustomize.git"       
        }
      }
    }
  }
